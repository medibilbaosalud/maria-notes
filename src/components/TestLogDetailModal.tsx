    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            ...
            >
                ...
                <div className="modal-body">
                    ...
                    <div className="metrics-grid">
                        ...
                        <div className="metric-card">
                            <div className="metric-icon purple"><Cpu size={20} /></div>
                            <div className="metric-info">
                                <label>Modelos Usados</label>
                                <span className="model-tag">{log.metadata.models.generation}</span>
                            </div>
                        </div>
                        <div className={`metric-card ${log.metadata.active_memory_used ? 'gold-glow' : ''}`}>
                            <div className={`metric-icon ${log.metadata.active_memory_used ? 'gold' : 'gray'}`}>
                                <Brain size={20} />
                            </div>
                            <div className="metric-info">
                    ...
                    <div className="validation-log-list">
                        {log.metadata.validationHistory && log.metadata.validationHistory.length > 0 ? (
                            log.metadata.validationHistory.map((error, idx) => (
                                <div key={idx} className="validation-item">
                                    <div className="error-badge">
                                        {error.type === 'hallucination' ? 'ALUCINACIÓN' :
                                            error.type === 'missing' ? 'FALTANTE' : error.type.toUpperCase()}
                                    </div>
                            ...
                            <div className="clean-run">
                                <CheckCircle size={40} />
                                <p>Ejecución Limpia: 0 Errores Detectados</p>
                            </div>
                        )}
                    </div>

                    {/* TRANSCRIPTION VS RESULT TABS (Simplified) */}
            ...
            <style>{`
                ...
                .header-title h2 { margin: 0; color: #1e293b; font-size: 1.25rem; }
                .date-badge { font-size: 0.85rem; color: #64748b; font-weight: 500; }
                .close-btn {
                    background: none;
                    border: none;
                    color: #64748b;
                ...
                .metric-icon.gold { background: #fef9c3; color: #ca8a04; }
                .metric-icon.gray { background: #e2e8f0; color: #94a3b8; }
                
                .gold-glow { border-color: #fde047; background: #fefce8; box-shadow: 0 0 10px rgba(234, 179, 8, 0.1); }

                .metric-info { display: flex; flex-direction: column; }
                .metric-info label { font-size: 0.75rem; color: #64748b; font-weight: 600; text-transform: uppercase; }
                .metric-info strong { font-size: 1rem; color: #0f172a; }
                .model-tag { font-size: 0.75rem; background: #e2e8f0; padding: 2px 6px; border-radius: 4px; font-family: monospace; }
                ...
                .section-title h3 { margin: 0; font-size: 1rem; color: #334155; }
                ...
                .error-content { flex: 1; font-size: 0.9rem; color: #334155; }
                .status-fixed {
                    font-size: 0.75rem;
                    color: #16a34a;
                    display: flex;
